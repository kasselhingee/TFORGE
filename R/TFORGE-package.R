...
#'
#' Additionally [`test_unconstrained_aGOE()`] can perform \eqn{2}-sample tests using Gaussian Orthogonal Ensemble approximation \insertCite{schwartzman2010gr}{TFORGE}.
#'
#' The above tests all require that the eigenvalues of the population mean are distinct (with decreasing performance when eigenvalues are very close).
...
#'
#' The package includes `scel.R` for empirical likelihood by  \insertCite{owen:2013;textual}{TFORGE}, which is used to estimate optimal non-uniform sampling weights for samples of constrained matrices.
#' 
#' This package was written on Ngunnawal and Ngambri Country. I thank the Country for its support.
#' @references
#' \insertAllCited{}
"_PACKAGE"
