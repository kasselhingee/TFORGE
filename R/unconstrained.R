...
stat_unconstrained <- function(x, evals = NULL, evecs = NULL, NAonerror = FALSE){
  ...
  # null evals
  if (is.null(evals)){
    d0 <- est_commonevals(x, Vs = Vs, evecs = evecs, NAonerror = NAonerror)
  } else {
...
#' Vs generated by applying cov_evals() to each sample
#' @export
est_commonevals <- function(mss, Vs, evecs = NULL, NAonerror = FALSE){
