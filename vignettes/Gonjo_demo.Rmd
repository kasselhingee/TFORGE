...
Since the matrices are AMS tensors, the traces are fixed to `1` and the eigenvalues are non-negative, so we can use `test_multiplicity_nonnegative()` and `test_fixedtrace()` to test some hypotheses relevant to AMS data.
...
Each interval has over 100 specimens. For fast computation in this demonstration we will use only the first 30 specimens of each interval.
...
```{r}
test_multiplicity(samples[["1"]], mult = c(2, 1), B = 1000)
test_multiplicity(samples[["2"]], mult = 3, B = 1000)
test_multiplicity(samples[["3"]], mult = c(1, 2), B = 1000)
```
