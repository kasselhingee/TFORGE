...
test_that("S_anv() gives good distribution of Tstat for diagonal covariances", {
  skip("Schwartzman's distribution approximation using two moments and projection onto a manifold doesn't appear to converge with increasing sample sizes.")
  p <- 3
  C2 <- C1 <- diag(p*(p+1)/2)
  ...
  anv <- S_anv(500, 1000, mn1, mn2, 
               ...
               C2 = diag(vecd(invvech(diag(C1))))) #the extra work accounts for strange vecd of Schwartzman
